...
DataManager.makeSavefileInfo = function() {
	...
	//记录角色等级
	info.actorName		= $gameActors.actor(1).name();
	info.actorLevel		= $gameActors.actor(1).level;
    return info;
};
...
ImageManager.loadPicture = function(filename, hue, m, i, c) {
	filename = filename.replace(/\/\//g, '/');
    return this.loadBitmap('img/pictures/', filename, hue, true, m, i, c);
};
...
ImageManager.loadBitmap = function(folder, filename, hue, smooth, m, i, c) {
    if (filename) {
        var path = folder + filename + '.png';
		var bitmap = this.loadNormalBitmap(path, hue || 0, m, i, c);
...
ImageManager.loadNormalBitmap = function(path, hue, m, i, c) {
    var key = path + ':' + hue;
    if (!this._cache[key]) {
        var bitmap;
		bitmap = Bitmap.load(path);
        bitmap.addErrorListener(function() {
			$LeNotifManager.add(path);
        });
...
Object.defineProperties(TextManager, {
    ...
    playerInfo      : TextManager.getter('command', 26),
    playerTitle     : TextManager.getter('command', 27),
...
BattleManager.initMembers = function() {
	...
	//wolfzq追加
	//技能出招前判定
	this._beforeAction = false;
	//技能出招后，不满足条件终止技能
	this._stopAction = false;
};
...
BattleManager.canEscape = function() {
	if (!this._canEscape) {
		...
		if (actor.isStateAffected(17)) { return false; }
		if (actor.isStateAffected(18)) { return false; }
	}
    return true;
};
    ...
    if (action) {
		if (this._beforeAction) {
			...
			if (action.isValid()) {
				...
				//wolfzq记录对象
				$wb._subject = this._subject;
				delete $wb._targets;
				$wb._targets = this._targets.clone();
				if (this._targets.length == 1 && this._subject.isActor() && this._subject.index() == 0) {
					$wb.targetIndex = this._targets[0].index();
				}
				var item = action._item;
				if (item) {
					var bSkill = item.isSkill();
					if (bSkill) {
						var aid = item.object().id;
						if (aid > 100) {
							$gameVariables.setValue(101, aid);
			...
			if ($gameSwitches.value(101)) {
				...
				this.refreshStatus();
				this._logWindow.startAction(subject, this._action, this._targets);
				subject.removeCurrentAction();
...
/*BattleManager.processTurn = function() {
	...
	if ($gameSwitches.value(101)) {
		...
		//wolfzq获得当前目标
		if (action) {
			//wolfzq获取技能编号100以上为H招式，调用公共事件101
			var bSkill = action._item.isSkill();
			if (bSkill) {
				var aid = action._item.object().id;
				if (aid > 100 && !this._beforeAction) {
					$gameVariables.setValue(101, aid);
					bSkill = true;
				} else {
					bSkill = false;
...
BattleManager.updateBattleEnd = function() {
    ...
    this._phase = null;
	//胜利时，拟态自动恢复装备
	if (this._result == 0 && $we.isMimicry()) {
		$gamePlayer.requestAnimation(102);
		$gameParty.loadWardrobe(5);
	}
};
