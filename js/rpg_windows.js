...
Window_Base.prototype.convertEscapeCharacters = function(text) {
	...
	//用W[]调用各种自定义文字
    text = text.replace(/\x1bW\[(\d+)\]/gi, function() {
        return $w.getNames(parseInt(arguments[1]));
    }.bind(this));
...
Window_Base.prototype.drawFace = function(faceName, faceIndex, x, y, width, height) {
	...
	//wolfzq追加代码
	var name = faceName.substr(0, 6);
	var pw = Window_Base._faceWidth;
	var ph = Window_Base._faceHeight;
	var dx = Math.floor(x + Math.max(width - pw, 0) / 2);
	var dy = Math.floor(y + Math.max(height - ph, 0) / 2);
	if (name == 'lilina') {
		var body = new Window_Body(0, true);
		dx += 75;
		dy += 518;
		body.x = dx;
		body.y = dy;
		...
		var bitmap = ImageManager.loadPicture(faceName + '_' +faceIndex);
		var sx = 178;
		var sy = 20;
		switch (faceName) {
			case 'Actor1':
				if (faceIndex == 0 || faceIndex == 3) { sy += 20; }
				else if (faceIndex == 4) { sx += 5; sy += 35; }
				else if (faceIndex == 5) { sx += 10; sy -= 20; }
				break;
			case 'Actor2':
				sy = 0;
				break;
...
Window_Base.prototype.drawActorIcons = function(actor, x, y, width) {
	//wolfzq修正状态图标为叠层显示
    width = width || 144;
	var maxLen = Math.floor(width / Window_Base._iconWidth);
	var icons = actor.allIcons();
	var len = icons.length;
	if (len > maxLen) {
		var maxWidth = Math.floor(Window_Base._iconWidth * maxLen / len);
		for (var i = 0; i < len; i++) {
			this.drawIcon(icons[i], x + maxWidth * i, y + 2);
		}
	} else {
		//var icons = actor.allIcons().slice(0, maxLen);
		for (var i = 0; i < len; i++) {
...
Window_Base.prototype.drawStateName = function(state, x, y, width) {
    width = width || 312;
    if (state) {
        var iconBoxWidth = Window_Base._iconWidth + 4;
        this.resetTextColor();
        this.drawIcon(state.iconIndex, x + 2, y + 2);
        this.drawText(state.name, x + iconBoxWidth, y, width - iconBoxWidth);
    }
};
...
Window_SkillList.prototype.drawItem = function(index) {
    ...
    if (skill) {
        ...
        this.drawItemName(skill, rect.x, rect.y, rect.width - costWidth - 60);
...
Window_EquipStatus.prototype.initialize = function(x, y, width) {
...
Window_EquipSlot.prototype.drawItem = function(index) {
    if (this._actor) {
        ...
        this.drawItemName(this._actor.equips()[index], rect.x + 138, rect.y, 240);
        this.changePaintOpacity(true);
    }
};
...
Window_EquipItem.prototype.includes = function(item) {
	...
	//wolfzq追加修正 if (this._slotId < 0 || item.etypeId !== this._actor.equipSlots()[this._slotId]) {
    if (this._slotId < 0 || !item || item.etypeId !== this._actor.equipSlots()[this._slotId]) {
        return false;
    }
	//wolfzq追加特殊装备处理机制
	if (this._slotId == 3) {
		if ($gameSwitches.value(47)) {
			var clothes = [0, 32, 55, 63, 82, 103, 105, 108, 127, 129, 150, 156, 174, 181, 227, 243, 248, 254, 305, 313, 319, 322, 346, 402, 422, 441, 448];
			if (!clothes.contains(item.id)) { return false; }
		}
	}
...
Window_Status.prototype.drawBlock1 = function(y) {
	...
	this.drawActorNickname(this._actor, 390, y, 138);
	if (this._actor.actorId() == 1) {
		this.drawParamsText($w.getJob($gameVariables.value(15)), 552, y, 168, this.textColor(4));
		this.drawParamsText($w.getSexJob($w.getNum('sexJob')), 744, y, 168, this.textColor(5));
		if ($gameVariables.value(1) == 0) {
			this.drawParamsText($w.getT('p7'), 936, y, 168, this.textColor(6));
		} else {
			this.drawParamsText($w.getT('p16'), 936, y, 168, this.textColor(27));
		}
	}
};
...
Window_Status.prototype.drawBlock3 = function(y) {
    ...
    this.drawEquipments(328, y);
    this.drawState(888, y);
};
...
Window_Status.prototype.drawEquipments = function(x, y) {
    ...
    for (var i = 0; i < count; i++) {
        this.drawItemName(equips[i], x + 300 * (i % 2), y + this.lineHeight() * parseInt(i / 2), 300);
    }
};

Window_Status.prototype.drawState = function(x, y) {
	var states = this._actor.states();
	var count = Math.min(states.length, this.maxEquipmentLines() / 2);
    for (var i = 0; i < count; i++) {
        this.drawStateName(states[i], x, y + this.lineHeight() * i);
    }
};
...
// Window_Language
//
// The window for changing various settings on the language screen.
...
Window_Language.prototype.addGeneralOptions = function() {
    this.addCommand('English',		'');
    this.addCommand('Japanese',		'JP');
    this.addCommand('中文版',		'CH');
};
...
Window_SavefileList.prototype.drawContents = function(info, rect, valid) {
    ...
    if (y2 >= lineHeight) {
        this.drawSavetime(info, rect.x, rect.y, rect.width);
...
Window_SavefileList.prototype.drawGameTitle = function(info, x, y, width) {
	if (info.mapName) {
        this.drawText(info.actorName + " " + TextManager.basic(0) + info.actorLevel + " " + info.mapName, x, y, width);
    } else {
...
Window_SavefileList.prototype.drawSavetime = function(info, x, y, width) {
    if (info.timestamp) {
		var saveTime = new Date(info.timestamp); 
        this.drawText(saveTime.Format("yyyy-MM-dd hh:mm"), x, y, width, 'right');
    }
};
...
Window_ShopStatus.prototype.drawActorParamChange = function(x, y, actor, item1) {
	...
	var width = this.contents.width - this.textPadding() - x;
	var txt = '';
	var name = ['MH', 'MM', 'MA', 'MD', 'AT', 'DF', 'SP', 'LK'];
	for (var i = 0; i < name.length; i++) {
		var change = this._item.params[i] - (item1 ? item1.params[i] : 0);
		if (change == 0) {
			continue;
		}
		txt += name[i] + (change > 0 ? '+' + change : '-' + (-change)) + ' ';
	}
    this.drawText(txt, x + 178, y, width - 178, 'right');
};
...
Window_Message.prototype.isTriggered = function() {
    return (Input.isRepeated('ok') || Input.isRepeated('cancel') || Input.isRepeated('skip') || 
...
Window_Message.prototype.loadMessageFace = function() {
	...
	if (faceName != '') {
		...
		if (name == 'lilina') {
			...
			this._faceBitmap = ImageManager.loadPicture(faceName + '_' + $gameMessage.faceIndex());
		}
	}
};
...
Window_ScrollText.prototype.updateMessage = function() {
    ...
    if (this.isSkip()) {
        this.origin.y = this.contents.height;
    }
...
Window_ScrollText.prototype.isSkip = function() {
    if ($gameMessage.scrollNoFast()) {
        return false;
    } else {
        return (Input.isRepeated('skip') || TouchInput.isRepeated());
...
Window_ActorCommand.prototype.processOk = function() {
    if (this._actor) {
        if (ConfigManager.commandRemember) {
			...
			if (symbol == 'skill') {
				this._actor._skillIndex = this.index();
			}
			this._actor._skillIndex = this.index();
        } else {
...
Window_ActorCommand.prototype.selectLast = function() {
    ...
    if (this._actor && ConfigManager.commandRemember) {
		...
		if (symbol == 'skill') {
			var index = this._actor._skillIndex;
			if (index) {
				if (this._list[index].symbol == 'skill') {
					this.select(index);
				}
			}
		}
