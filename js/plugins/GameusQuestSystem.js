    ...
    Game_Interpreter.prototype.pluginCommand = function(command, args) {
        ...
        if (command.toLowerCase() === "quest") {
            ...
            case 'backstep':
                ...
                break;
            case 'setstep':
                $gameQuests.get(Number(args[1])).setStep(args[2]);
                break;
            }
        }
    };
...
//---------------------------------------------------------------------------------------------
    ...
    Game_Party.prototype.addQuest = function(quest_id) {
        ...
        if (this.quests.indexOf(quest_id) < 0) {
			...
			$gameQuests.get(quest_id).message();
        }
    };
...
//---------------------------------------------------------------------------------------------
    ...
    Game_Quest.prototype.reload = function() {
		//任务开发阶段调用，不再开发可以直接return;
        var questData = $dataQuests[this.questId];
        this.rawData = questData;
        this.cat = questData.cat;
        this.name = questData.name;
        this.desc = questData.desc;
	...
	//wolfzq提示
	Game_Quest.prototype.message = function(content) {
		if ($gameParty.hasItem($dataItems[14])) {
			$LeNotifManager.add($w.getT('p27') + this.name + $w.getT('p28'));
		}
    }
    ...
    Game_Quest.prototype.setStep = function(val) {
...
//---------------------------------------------------------------------------------------------
    Window_Base.prototype.sliceText = function(text, width) {
		...
		var bw = this.contents.measureTextWidth(" ");
        var current_text = "";
        for (var i = 0; i < len; i++) {
            var word = words[i];
			current_text += word;
            var textW = this.contents.measureTextWidth(current_text);
			if (word == '\n' || (word == ' ' && (textW + words.substr(i + 1).indexOf(' ') * bw) > width)) {
                result.push(current_text);
                current_text = "";
            } else if (textW > width && !/^[0-9a-zA-Z ,\.;\:"'!]+$/.test(word)) {
                result.push(current_text);
                current_text = "";
            }
            if (i >= words.length - 1)
...
//---------------------------------------------------------------------------------------------
    ...
    Window_QuestInfo.prototype.setQuest = function(quest_id) {
        this.quest = quest_id;
		if (quest_id > -1) {
			$gameParty.quest_id = quest_id;
		}
        this.offY = 0;
        this.questBitmap.resize(this.contentsWidth(), this.contentsHeight());
        this.resizeFlag = false;
    ...
    Window_QuestInfo.prototype.refresh = function() {
        ...
        if (this.quest > 0) {
			...
			if (this.offY + height > this.questBitmap.height) {
				height = this.questBitmap.height - this.offY;
			}
            this.contents.blt(this.questBitmap, 0, this.offY, this.contentsWidth(), height, 0, 0, this.contentsWidth(), this.contentsHeight());
        }
    }
    ...
    Window_QuestInfo.prototype.write = function() {
        ...
        if (this.lineY > this.questBitmap.height) {
            this.questBitmap.resize(this.questBitmap.width, this.lineY + 20);
    ...
    Window_QuestInfo.prototype.drawQuestInfo = function(q) {
        ...
        for (var i = 0; i < lines.length; i += 1) {
            var lineText = this.convertEscapeCharacters(lines[i]);
            this.questBitmap.drawText(lineText, 0, this.lineY, this.contentsWidth(), this.lineHeight());
            this.write();
        }
    ...
    Window_QuestInfo.prototype.drawQuestSteps = function(q) {
        ...
        this.questBitmap.textColor = this.systemColor(); 
		this.questBitmap.drawText($w.getT("Steps"), 0, this.lineY, this.contentsWidth(), this.lineHeight());
        ...
        var startStep = 0;
		//Math.max(0, q.currentStep - maxSteps + 1);
        var drawableSteps = q.steps.slice(startStep, q.currentStep + 1);
        for (var i = drawableSteps.length - 1; i >= 0; i -= 1) {
            var step = drawableSteps[i];
            var stepText = bullet + step[0];
            stepText = this.convertEscapeCharacters(stepText);
            if (step[1] === true) {
				var varVal;
				if(/^\d+$/.test(step[2])) {
					varVal = $gameVariables.value(step[2]);
				} else {
					varVal = $w.getNum(step[2]);
				}
                var maxVal = step[3];
                if (step[4])
    ...
    Window_QuestInfo.prototype.drawQuestRewards = function(q) {
		...
		if (len > 0) {
			this.questBitmap.textColor = this.systemColor(); 
			this.questBitmap.drawText($w.getT("Rewards"), 0, this.lineY, this.contentsWidth(), this.lineHeight());
...
//---------------------------------------------------------------------------------------------
    ...
    Window_Quests.prototype.drawItem = function(index) {
        ...
        if (item.symbol === "quest") {
            ...
            if (q.icon > -1 && (GameusScripts["Config"]["QuestSystem"]["Use Icons"]).toLowerCase() === "true") {
                this.drawIcon(q.icon, rect.x + 2, rect.y + 2);
                tempX = 36;
            }
        }
        this.drawText(this.commandName(index), tempX, rect.y, (rect.width - tempX), "left");
    };
    ...
    Window_Quests.prototype.refreshQuests = function() {
		...
		var vc = 0;
		this.oldIndex = 0;
		var questId = $gameParty.quest_id || -1;
        for (var i = 0; i < this.cats.length; i += 1) {
            this.expanded[i] = false;
            this.counter[i] = 0;
            for (var j = 0; j < this.data.length; j += 1) {
                ...
                if (q.cat == i && (this.filter == q.status || this.filter == "all")) {
                    this.counter[i] += 1;
					if (this.data[j] == $gameParty.quest_id) {
						this.expanded[i] = true;
						this.oldIndex = vc + this.counter[i];
					}
				}
            }
			if (this.counter[i] > 0) {
        ...
        if (useCats === "true") {
            for (var i = 0; i < this.cats.length; i += 1) {
                ...
                if (this.expanded[i]) {
                    ...
                    if (!flag)
                        this.addCommand("  " + $w.getT("NoQuest"), "none");
                }
            }
        } else {
    ...
    Window_Quests.prototype.cursorRight = function(wrap) {
        $gameParty.quest_filter = this.filterIndex + 1 > 3 ? 0 : this.filterIndex + 1;
		this.selectFilter();
    };
 
    Window_Quests.prototype.cursorLeft = function(wrap) {
        $gameParty.quest_filter = this.filterIndex - 1 < 0 ? 3 : this.filterIndex - 1;
		this.selectFilter();
    };
 
    Window_Quests.prototype.selectFilter = function() {
        this.filterIndex = $gameParty.quest_filter || 0;
        this.filter = this.qFilters[this.filterIndex];
        this.refreshQuests();
        this.refresh();
        this.select(this.oldIndex);
	}
//---------------------------------------------------------------------------------------------
// Window_QuestFilter
//---------------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------------------
    ...
    Scene_Quest.prototype.createQuestWindow = function() {
		...
		//加载记录中的任务默认信息
		this.questWindow.selectFilter($gameParty.quest_filter);
    };
