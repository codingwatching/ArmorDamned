...
(function(){
	...
	var categories = wolfzqText['p20'].replace(/^\s+|\s+$/gm,'').split(',');
	...
	var priceText = wolfzqText['p21'];
    var equipText = wolfzqText['p22'];
    var typeText = wolfzqText['p23'];
    var ingredientsText = wolfzqText['p24'];
	var currencyText = String(parameters['Currency Text'] || 'Currency');
	var obsceneText = wolfzqText['obscene'];
	var itemCraftedText = wolfzqText['p25'];
		...
		dbs.forEach(function(db){
			db.filter(function(obj){return obj != null;}).forEach(function(item){
				...
				if (item.note.match(/<recipe>[\s\S]*<\/recipe>/gim)){
					...
					item.note.match(/([iwa]:\s*\d+,\s*\d+|[co]:\s*\d+)/gim).forEach(function(ing_line){
						var ing = ing_line.match(/([iwa]):\s*(\d+),\s*(\d+)|([co]):\s*(\d+)/i);
						var ing_db;
						var needed;
						if (ing[1] != null){
							switch (ing[1]){
						...
						if (ing_db == obsceneText || ing_db == currencyText){
	...
	*---------------------------------------------*/
	...
	Scene_CraftingMenu.prototype.create = function(){
		...
		// Create item window
		this._indexWindow = new Window_CraftingItems(0, this._helpWindow.height, 288, Graphics.height-this._helpWindow.height, this._category);
		...
		this._statusWindow = new Window_CraftingItemStatus(this._indexWindow.width, this._helpWindow.height, Graphics.width-this._indexWindow.width);
		this._statusWindow._helpWindow = this._helpWindow;
	...
	Scene_CraftingMenu.prototype.loseIngredients = function(){
		var item = this._indexWindow.item();
       	...
       	item._ingredients.forEach(function(ing){
       		needed = ing['amount'];
			if (ing['item'] == obsceneText) {
				$w.addP('obscene', -needed);
			} else if (ing['item'] == currencyText){
    ...
    Window_CraftingItemStatus.prototype.refresh = function() {
        ...
        if (DataManager.isWeapon(item) || DataManager.isArmor(item)) {
            ...
            for (var i = 2; i < 8; i++) {
                ...
                this.resetTextColor();
                this.drawText(item.params[i], x + 120, y, 120, 'right');
                y += lineHeight;
            }

            this.changeTextColor(this.systemColor());
            this.drawText(TextManager.param(0), x, y, 120);
            this.resetTextColor();
            this.drawText(item.params[0], x + 120, y, 120, 'right');
            y += lineHeight;

            this.changeTextColor(this.systemColor());
            this.drawText(TextManager.param(1), x, y, 120);
            this.resetTextColor();
            this.drawText(item.params[1], x + 120, y, 120, 'right');
        }
    };
