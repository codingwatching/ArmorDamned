...
Game_Timer.prototype.work = function() {
	if (this._frames > 0) {
		this._working = true;
	}
};
...
Game_Switches.prototype.setValue = function(switchId, value) {
    if (switchId > 0 && switchId < $dataSystem.switches.length) {
		...
		$we.switchListener(switchId, value);
        this.onChange();
    }
};
...
Game_Screen.prototype.startZm = function(x, y, scale, duration) {
    this._zoomX = x;
    this._zoomY = y;
...
Game_BattlerBase.prototype.meetsSkillConditions = function(skill) {
    var bUse = (this.meetsUsableItemConditions(skill) &&
	...
	if (bUse) {
		switch (skill.id) {
			case 37:
				if ($w.getP('obscene') < 500) return false;
				break;
			case 38:
				if ($w.getP('obscene') < 5000 || !$gameSwitches.value(130)) 
					return false;
				break;
			case 40:
				if ($w.getP('fall') < 1000) 	return false;
				break;
			case 48:
				if ($w.getP('obscene') < 200) return false;
				break;
			case 49:
				if ($w.getP('obscene') < 50 || $gameSwitches.value(84)) return false;
				else if ($gameVariables.value(1) == 0 && $w.getP('lust') < 500 ) return false;
				break;
			case 58:
				if ($w.getP('obscene') < 50) return false;
				break;
			case 101:
				if (!$w.isStateOver(['h'], 1) || $w.getP('obscene') < 10 || $w.getP('fall') < 800)
					return false;
				break;
			case 102:
				if (!$w.isStateOver(['h'], 1) || $w.getP('obscene') < 5 || $w.getP('fall') < 150)
					return false;
				break;
			case 103:
				if (!$w.isStateOver(['h'], 2) || $w.getP('obscene') < 10 || $w.getP('fall') < 500)
					return false;
				break;
			case 104:
				if (!$w.isStateOver(['h'], 3) || $w.getP('obscene') < 20 || $w.getP('fall') < 1000)
					return false;
				break;
			case 105:
				if ($w.getP('obscene') < 10 || $w.getP('fall') < 150) 
					return false;
				break;
			case 106:
				if ($w.getP('obscene') < 100)
					return false;
				break;
			case 107:
				if ($w.getP('obscene') < 100) return false;
				break;
			case 108:
				if ($w.getP('obscene') < 10 || $w.getP('fall') < 1000) 
					return false;
				break;
			case 109:
				...
				break;
			case 111:
				if ($w.getP('obscene') < 30 || $w.getNum('sexJob4') == 0) 
					return false;
				break;
			case 112:
				if ($w.getP('obscene') < 50 || $w.getP('cursed') < 1000) 
					return false;
				break;
		}
...
Game_Battler.prototype.regenerateHp = function() {
    ...
    if (value !== 0) {
		if (value > 0 && BattleManager._phase && this.isActor() && this.isAlive() && this._actorId == 1) {
			//wolfzq追加回血效果特殊处理
			$wb._subject = this;
			$wb._targets = [this];
			var log = SceneManager._scene._logWindow;
			log.addText(this._name + $w.getT('p15'));
			log.showAnimation(this, [this], 46);
			log._waitCount += 100;
		}
        this.gainHp(value);
    }
...
Game_Actor.prototype.isEquipChangeOk = function(slotId) {
	...
	switch (slotId) {
		case 0:
			if ($gameSwitches.value(47)) { return false; } //拘束
		break;
		case 3:
			if ($w.getInfo('cloth') == '$cloth22v1' || $w.getInfo('cloth') == '$cloth22v2') { return false; }
		break;
		case 4:
			if ($w.getInfo('shirt') == 'rope2') { return false; } //绳束
		break;
		case 5:
			if ($gameSwitches.value(47)) { return false; } //拘束
		break;
...
Game_Map.prototype.autoplay = function() {
    if ($dataMap.autoplayBgm && !$gameSwitches.value(102)) {
...
Game_CharacterBase.prototype.initMembers = function() {
	...
	this._waitPattern = 0;	//允许帧停顿
    this._tmpX = 0;	//X轴偏移
    this._tmpY = 0;	//Y轴偏移
	this._alwaysMove = false; //始终向指定方向移动
	this._alwaysRoute = 6; //移动方向，默认为右
	this._moveStartX = -1; //移动开始坐标X
	this._moveEndX = -1; //移动达成坐标X
	this._moveRound = 0; //移动单次循环次数，
	//赛马用变量
	this._mSpeed = 0;
	this._tinySpeed = 0; //速度细微差_tinySpeed
};
...
Game_CharacterBase.prototype.distancePerFrame = function() {
    return Math.pow(2, this.realMoveSpeed()) / (256 - this._tinySpeed * 4);
};
...
Game_CharacterBase.prototype.update = function() {
    if (this.isStopping()) {
		if (this._alwaysMove) {
			if (this._moveEndX > -1 && this._x == this._moveEndX) {
				if (this._moveRound > 0) {
					this._moveRound --;
					var x = this._moveEndX;
					//更换坐标和方向
					this._moveEndX = this._moveStartX;
					this._moveStartX = x;
					this._alwaysRoute = this._alwaysRoute == 6 ? 4 : 6;
				} else {
					this._alwaysMove = false;
					this.updateStop();
				}
			} else {
				this.moveStraight(this._alwaysRoute);
				this.changeMSpeed();
			}
		} else {
			this.updateStop();
...
//跑马速度微调改变
Game_CharacterBase.prototype.setMSpeed = function() {
	//改变速度
	if (this._alwaysRoute == 6) {
		this._changeSX = this._x + 8;
	} else if (this._alwaysRoute == 4) {
		this._changeSX = this._x - 8;
	} else {
		this._changeSX = 0;
	}
	if (this._mSpeed > $w._fast - 3 && Math.random() > 0.7) {
		//最快的马，则有一定概率丧失速度，主角则根据好感度判定是否丧失速度
		if (this === $gamePlayer && $w.getNum('djsLover') - Math.random() * 3000 > 0){
			this._tinySpeed = Math.floor(this._mSpeed + 10 * Math.random());
		} else {
			this._tinySpeed = 0;
		}
	} else {
...
Game_CharacterBase.prototype.changeMSpeed = function() {
	//改变速度，并且设定下次改变速度的临界值
	if (this._alwaysRoute == 6 && this._x > this._changeSX) {
		this.setMSpeed();
	} else if (this._alwaysRoute == 4 && this._x < this._changeSX) {
		this.setMSpeed();
	}
};
...
Game_CharacterBase.prototype.setWaitPattern = function(val) {
	this._waitPattern = val;
}

Game_CharacterBase.prototype.updatePattern = function() {
	if (this._waitPattern == 1) {
...
Game_CharacterBase.prototype.pattern = function() {
    return this._pattern < 4 ? this._pattern : 0;
};
...
Game_Player.prototype.wakeUp = function() {
	if (this._isSleep) {
...
Game_Player.prototype.wantSleep = function() {
	//wolfzq追加原地犯困脚本
	if ($gameActors.actor(1).isStateAffected(47)) {
		this._wantSleep = 600;
	} else {
		this._wantSleep = 3600;
	}
...
Game_Player.prototype.moveByInput = function() {
    if (!this.isMoving() && this.canMove()) {
		...
		//动画效果
		if (this._wantSleep > 0) {
			if (!$gameSwitches.value(85) && !$gameSwitches.value(161)) {
				this._wantSleep--;
			}
		} else {
...
Game_Player.prototype.updateVehicleGetOff = function() {
    if (!this.areFollowersGathering() && this.vehicle().isLowest()) {
		...
		if ($gameSwitches.value(161)) {
			//wolfzq飞行着陆隐藏飞行器
			$gameSwitches.setValue(161, false);
			$we.updateTile(true);
			$gameMap._vehicles[2].setImage('', 0);
			$gameMap._vehicles[2].setPosition(0, 0);
		}
    }
};
...
Game_Player.prototype.getOnVehicle = function() {
    ...
    } else if ($gameSwitches.value(161) && $gameMap._vehicles[2]._mapId == $gameMap.mapId()) {
		//wolfzq飞行器
		this._vehicleType = 'airship';
		this.vehicle().setPosition(x1, y1);
		this.vehicle().setImage(this.characterName(), this.characterIndex());
	}
    if (this.isInVehicle()) {
...
Game_Player.prototype.getOffVehicle = function() {
    if (this.vehicle().isLandOk(this.x, this.y, this.direction())) {
        if (this.isInAirship() && !$gameSwitches.value(161)) {
			//wolfzq飞行器方向不变
            this.setDirection(2);
        }
...
//重玩家载函数
//wolfzq追加跑步标志，判断是否可以开启跑步模式, 默认1；允许跑步
Game_Player.prototype._isRun = 1;
Game_Player.prototype.setIsRun = function(isRun) {
	this._isRun = isRun;
}
Game_Player.prototype.isRun = function() {
	return this.isDashing() * this._isRun;
}
Game_Player.prototype.realMoveSpeed = function() {
	var cn = this.characterName();
	var cn6 = cn.substr(0, 6);
	...
	if (!$gameMap.isAnyEventStarting()) {
		...
		if (!bHorse) {
			...
			//铁球速度-1，爬行速度-1，寄生虫-1，魔钢拘束器硬化后速度-2
			if ($w.getInfo('item1') == 'ball6') {
				speed -= 1;
			}
			if ($gameSwitches.value(121)) {
				speed -= 1;
			}
			if ($w.getNum('parasite') > 0) {
				speed -= 1;
			}
			if (cn.substr(0, 7) == 'lilina2' && index == 2) {
				speed -= 2;
			}
		}

		if (cn6 != 'lilina') {
			this.setImage(cn, index);
			//以下点图不能忽略速度修正
			if (!bHorse && cn6 != 'parasi') {
				speed = 0;
			}
		} else {
			if (this.isRun() && $w.getNum('gravidity') == 0) {
				if (cn.substr(cn.length - 3, 3) != 'pao') {
					this.setImage(cn + 'pao', index);
				}
				speed += 1;
			} else {
				if (cn.substr(cn.length - 3, 3) == 'pao') {
					this.setImage(cn.substr(0, cn.length - 3), index);
				}
			}
		}
	} else {
...
Game_Interpreter.prototype.command101 = function() {
    if (!$gameMessage.isBusy()) {
		...
		var bSkip = $gameSwitches.value(171) && $gameMap.isEventRunning() && !SceneManager.isSceneChanging() && Input.isLongPressed('skip');
		var tmpIndex = this._index;
		...
		//极速跳过，如果极速的最后是操作，那么回到最开始点显示
		if (bSkip) {
			switch (this.nextEventCode()) {
			case 102:  // Show Choices
			case 103:  // Input Number
			case 104:  // Select Item
				this._index = tmpIndex;
				while (this.nextEventCode() === 401) {  // Text data
					this._index++;
					$gameMessage.add(this.currentCommand().parameters[0]);
				}
