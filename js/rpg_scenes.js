...
Scene_Boot.prototype.updateDocumentTitle = function() {
	...
	document.title = $dataSystem.gameTitle + $w.getT('version');
};
...
Scene_Title.prototype.commandReLogin = function() {
	SceneManager.login();
};
...
Scene_Title.prototype.DisplayCustomText = function() {
	...
	var y = Graphics.height - font_size;
	var maxWidth = Graphics.width;
	this._titleCustomText.bitmap.outlineColor = "#000000";
	this._titleCustomText.bitmap.outlineWidth = 2;
	this._titleCustomText.bitmap.fontSize = font_size;
	this._titleCustomText.bitmap.textColor = "#FFFFFF";
	this._titleCustomText.bitmap.drawText('version ' + $w.getT('version'), -10, y, maxWidth, font_size, 'right');
};
...
Scene_Map.prototype.updateMainMultiply = function() {
    ...
    if (this.isFastForward()) {
		//场景执行频率，增加updateMain执行次数可加速场景速度
        this.updateMain();
    } else if (this.isSkipForward()) {
		for (var i = 0; i < 10; i++) {
			this.updateMain();
			AudioManager.stopSe();
		}
	} else if (!$gameMap.isEventRunning() && !SceneManager.isSceneChanging()) {
		if (this.isWardrobe()) {
...
Scene_Map.prototype.isWardrobe = function() {
    return Input.isTriggered('wardrobe');
};
Scene_Map.prototype.isMimicry = function() {
    return Input.isTriggered('mimicry');
};
Scene_Map.prototype.isUseSkill = function() {
    return Input.isTriggered('useskill');
};
...
Scene_Map.prototype.createSpriteset = function() {
	...
	//立绘层
    this._bodyWindow = new Window_Base();
    this.addChild(this._bodyWindow);
	//CG层
    this._cgWindow = new Window_Base();
    this.addChild(this._cgWindow);
    this._cgWindow.addChild(this._spriteset._pictureContainer);
};
...
Scene_Map.prototype.touchMove = function() {
	if ($gamePlayer.touchMove && TouchInput.x < 568) {
		if (TouchInput.isReleased()) {
			$we.delButton(91);
			$we.addButton(90, 'move', 150, 490, 200, 200, 100);
			$gamePlayer.touchMove = false;
		} else {
			var x = 150;
			var y = 490;
			var b = (TouchInput.x - x) * (TouchInput.x - x) + (TouchInput.y - y) * (TouchInput.y - y);
			var a = Math.sqrt(10000 / b);
			var x1 = (TouchInput.x - x) * a + x;
			var y1 = (TouchInput.y - y) * a + y;
			if ((x - TouchInput.x) * (x - TouchInput.x) + (y - TouchInput.y) * (y - TouchInput.y) < 10000) {
				x1 = TouchInput.x;
				y1 = TouchInput.y;
			}
			$we.addButton(90, 'move', 150, 490, 200, 200, 200);
			$we.addButton(91, 'move', x1, y1, 50, 50);
			if (!$gamePlayer.isMoving() && $gamePlayer.canMove()) {
				var sx = $gameScreen.picture(91)._x;
				var sy = $gameScreen.picture(91)._y;
				if (sx + sy > 640) {
					if (sx - sy < -340) {
...
Scene_Skill.prototype.update = function() {
	...
	if ($w.getNum('skillButton') == 1) {
		$w.setNum('skillButton', 0);
		if ($gameSwitches.value(1) && this._skillTypeWindow._index != 1) {
			this._skillTypeWindow.select(1);
		}
		this._skillTypeWindow.processOk();
	}
};
...
Scene_Equip.prototype.createFittingRoomWindow = function() {
    this._fittingRoomWindow = new Window_FittingRoom(Graphics.boxWidth - 380, 0);
    this.addWindow(this._fittingRoomWindow);
	this._fittingRoomWindow.refresh();
};

Scene_Equip.prototype.createHelpWindow = function() {
    this._helpWindow = new Window_Help(2, Graphics.boxWidth - this._fittingRoomWindow.width);
    this.addWindow(this._helpWindow);
};
...
Scene_Equip.prototype.createCommandWindow = function() {
    ...
    var wy = this._helpWindow.height;
    var ww = Graphics.boxWidth - this._statusWindow.width - this._fittingRoomWindow.width;
...
Scene_Equip.prototype.createSlotWindow = function() {
    ...
    this._slotWindow = new Window_EquipSlot(wx, this._helpWindow.height, ww, wh);
...
Scene_Equip.prototype.createItemWindow = function() {
    var wx = 0;
    var wy = this._statusWindow.y + this._statusWindow.height;
    var ww = Graphics.boxWidth - this._fittingRoomWindow.width;
    var wh = Graphics.boxHeight - wy;
...
Scene_Equip.prototype.refreshActor = function() {
    ...
    this._itemWindow.setActor(actor);
    this._fittingRoomWindow.setActor(actor);
...
// Scene_Language
//
// The scene class of the language screen.
...
Scene_Language.prototype.createOptionsWindow = function() {
    this._optionsWindow = new Window_Language();
    this._optionsWindow.setHandler('cancel', this.popScene.bind(this));
    this._optionsWindow.setHandler('',  this.en.bind(this));
    //this._optionsWindow.setHandler('JP',  this.jp.bind(this));
    //this._optionsWindow.setHandler('CH',  this.ch.bind(this));
    this.addWindow(this._optionsWindow);
};

Scene_Language.prototype.en = function() {
	StorageManager.save('language', '');
	SceneManager.goto(Scene_Boot);
}
Scene_Language.prototype.jp = function() {
	StorageManager.save('language', 'JP');
	SceneManager.goto(Scene_Boot);
}
Scene_Language.prototype.ch = function() {
	StorageManager.save('language', 'CH');
	SceneManager.goto(Scene_Boot);
}
...
Scene_Battle.prototype.updateContent = function() {
	var active = this.isActive();
	$gameTimer.update(active);
	$gameScreen.update();
